...
def snapshot_gen(G, tot_host_num, src_num, migr_type):
    ...
    for i in range(tot_host_num):
        if i < src_num: # SRC 
            ...
            src_upBWC = random.uniform(src_upBWC_range)
            src_dnBWC = random.uniform(src_dnBWC_range)
            src_sigmaC = random.uniform(src_sigmaC_range)
            
            while 1:
                vm_upSBW = random.uniform(vm_upSBW_range)
                vm_dnSBW = random.uniform(vm_dnSBW_range)
                vm_sigma = random.uniform(vm_sigma_range)
                vm_ori_size = random.uniform(vm_ori_size_range)
            ...
            dst_upBWC = random.uniform(dst_upBWC_range)
            dst_dnBWC = random.uniform(dst_dnBWC_range)
            dst_sigmaC = random.uniform(dst_sigmaC_range)
    ...
    if migr_type == 'Consolidation':
        migr_gen_C(vm_num_count, src_num, all_host__dict, all_VM__dict, src__dict)
    else # LB case
        migr_gen_LB()
    
    return all_VM__dict, all_host__dict
...
def migr_gen_C(vm_num_count, src_num, all_host__dict, all_VM__dict, src__dict):
    ...
    for aVM in vm__list:
        assert all_VM__dict[aVM].DSTnum != None, 'There is a VM havnt alloc to DST since all the DSTs are full'
